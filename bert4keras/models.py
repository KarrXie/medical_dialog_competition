...
class Dialog_Mask(object):
    ...
    def compute_attention_bias(self, inputs=None):
        ...
        if self.attention_bias is None:
            def unilm_mask(s):
                idxs = K.cumsum(s, axis=1)
                mask = (idxs[:,None,:] - idxs[:,:,None] >= -2) & (idxs[:,None,:] - idxs[:,:,None] <= 0)
                mask = K.cast(mask, K.floatx())
                return -(1 - mask[:, None]) * 1e12
...
class BERT(Transformer):
    ...
    def __init__(
        ...
        self.output_entity_logits = kwargs.get("output_entity_logits", False)
    ...
    def apply_final_layers(self, inputs):
        ...
        if self.with_mlm:
            ...
            logits_outputs = self.apply(inputs=x, layer=BiasAdd, name='MLM-Bias')
        ...
        if len(outputs) == 1:
            ...
            #     _logits_outputs = Lambda(lambda x: x[:, :, -len(self.compound_tokens):])(logits_outputs)
            #     outputs = [_logits_outputs, outputs[1]]
            # else:
                outputs = outputs[1]
            elif self.output_logits:
                if self.output_entity_logits:
                    # outputs = [outputs[1], outputs[1]]
                    _logits_outputs = Lambda(lambda x: x[:, :, -len(self.compound_tokens):])(logits_outputs)
                    # _logits_outputs = Lambda(lambda x: K.concatenate([x[:, :, 1:2], x[:, :, -len(self.compound_tokens):]], axis=-1))(logits_outputs)
                    # _logits_outputs = BiasAdd(name="entity_bias")(_logits_outputs)
                    _logits_outputs = self.apply(inputs=_logits_outputs, layer=BiasAdd, name='entity_bias')
                    outputs = [_logits_outputs, outputs[1]]
                else:
                    outputs = outputs[0]
        else:
...
def extend_with_dialog_language_model(BaseModel):
    """添加UniLM的Attention Mask（Seq2Seq模型用）
    """

    class DialogLanguageModel(Dialog_Mask, BaseModel):
        def __init__(self, *args, **kwargs):
            super(DialogLanguageModel, self).__init__(*args, **kwargs)
            self.with_mlm = self.with_mlm or True

    return DialogLanguageModel
...
def build_transformer_model(
    ...
    elif application == "dialog":
        MODEL = extend_with_dialog_language_model(MODEL)
