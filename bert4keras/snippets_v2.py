...
class AutoRegressiveDecoder(object):
    ...
    """
    def __init__(self, start_id, end_id, maxlen, minlen=1, thred=0):
    ...
    def random_sample(
        ...
        entity_results = []
        for step in range(self.maxlen):
            pred_out, states = self.predict(inputs, output_ids, states)  # 计算当前概率
            probas, entity_probas = pred_out[0], pred_out[1]
            probas /= probas.sum(axis=1, keepdims=True)  # 确保归一化
            if step == 0:  # 第1步预测后将结果重复n次
                entity_results.append(np.argmax(entity_probas))
            ...
            is_end = (output_ids[:, -1] == self.end_id and np.argmax(entity_probas) == entity_results[-1]) # 标记是否以end标记结束
            # if output_ids[:, -1] == self.end_id:
            #     print("***", max(entity_probas))
            if output_ids[:, -1] == self.end_id and not is_end:
                entity_results.append(np.argmax(entity_probas))
            end_counts = (output_ids == self.end_id).sum(1)  # 统计出现的end标记
            if output_ids.shape[1] >= self.minlen:  # 最短长度判断
